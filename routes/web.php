<?php

use App\Http\Controllers\HomeController;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', [HomeController::class, 'index'])->name('home');
Route::get('/contact', [HomeController::class, 'contact'])->name('contact');

Route::controller(\App\Http\Controllers\ServiceController::class)
    ->prefix('service')
    ->group(function () {
        Route::get('poster-design', 'poster')->name('poster');
        Route::get('package-design', 'package')->name('package');
        Route::get('ui-ux', 'ui')->name('ui');
        Route::get('brand-identity', 'brand')->name('brand');
        Route::get('details/{slug}', 'details')->name('details');
    });

Route::controller(\App\Http\Controllers\BlogController::class)
    ->prefix('blog')
    ->group(function () {
        Route::get('posts','index')->name('blog.posts');
        Route::get('details/{slug}','details')->name('details.post');
    });

Route::group(['prefix' => 'icreate-manage'], function () {
    Voyager::routes();
});
